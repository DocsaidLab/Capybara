...
def test_get_package_versions_basic():
    ...
    for package in expected_packages:
        ...
        assert version_key in versions or error_key in versions, \
            f"No information found for {package}"
...
@patch('capybara.utils.system_info.subprocess.run')
def test_get_gpu_cuda_versions_nvidia_smi_success(mock_run):
    """Test GPU CUDA version detection when nvidia-smi succeeds."""
    # Mock successful nvidia-smi output
    mock_run.return_value = MagicMock(
        returncode=0,
        stdout="CUDA Version: 11.8"
    )
    
    versions = get_gpu_cuda_versions()
    
    assert isinstance(versions, dict)
    # The function might return different key formats
    assert any("CUDA" in key for key in versions.keys())


@patch('capybara.utils.system_info.subprocess.run')
def test_get_gpu_cuda_versions_nvidia_smi_failure(mock_run):
...
def test_get_system_info_basic():
    ...
    expected_keys = ["OS Version", "CPU Model", "Physical CPU Cores", 
                    "Total RAM (GB)", "Disk Total (GB)"]
