...
from capybara import (ONNXEngineIOBinding, get_onnx_input_infos,
                      get_onnx_output_infos, make_onnx_dynamic_axes)
...
def test_get_onnx_output_infos():
    ...
    assert output_infos == {
        'output': {'shape': [1, 64, 56, 56], 'dtype': 'float32'}
    }
...
def test_make_onnx_dynamic_axes():
    ...
    input_dims = {
        k: {0: 'b', 2: 'h', 3: 'w'}
        for k in input_infos.keys()
    }
    output_dims = {
        k: {0: 'b', 2: 'h', 3: 'w'}
        for k in output_infos.keys()
    }
